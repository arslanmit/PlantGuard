...
class TestPlantGuardSPAUnit:
    ...
    def test_session_state_management(self, spa_instance):
        ...
        result = {"disease": "Test Disease", "confidence": 0.85, "recommendations": ["Test recommendation"]}
    ...
    def test_processing_state_updates(self, spa_instance):
        ...
        spa_instance.update_processing_state("processing", "image", {"current": 1, "total": 5})
...
class TestPlantGuardSPAProgrammaticAPI:
    ...
    def test_analyze_image_programmatic(self, spa_instance, sample_image_file):
        ...
        mock_result = {"disease": "Test Disease", "confidence": 0.85, "model": "vit_best", "recommendations": ["Test recommendation"]}
    ...
    def test_batch_analyze_programmatic(self, spa_instance, sample_image_file):
        ...
        mock_result = {"status": "success", "disease": "Test Disease", "confidence": 0.85}
...
class TestPlantGuardSPAResponsiveLayout:
    ...
    def spa_instance(self):
        ...
        # Initialize with basic required state to avoid errors
        try:
            init_session_state()
        except Exception:
            # If init fails, set basic required keys manually
            st.session_state.update(
                {
                    "analysis_history": [],
                    "chat_messages": [],
                    "comparison_mode": False,
                    "current_models": {"vision": "vit_best", "audio": "whisper_tiny_local", "text": "distilbert_plant_qa_v1"},
    ...
    def test_responsive_layout_rendering(self, spa_instance):
        """Test that responsive layout renders without errors."""
        # Should not raise exceptions when rendering responsive layout
        try:
            spa_instance.render_adaptive_layout()
            # If we get here, no exceptions were raised
            assert True
        except Exception as e:
            # If there are exceptions, they should be related to Streamlit context, not our code
            assert "ScriptRunContext" in str(e) or "session_state" in str(e)
...
class TestPlantGuardSPAPerformance:
    ...
    def test_large_history_management(self, spa_instance):
        ...
        for i in range(150):  # More than max_history (100)
            spa_instance.save_analysis_result({"disease": f"Disease_{i}", "confidence": 0.8}, f"test_{i}.jpg")
