...
STREAMLIT_WARN_FILTER := ignore::UserWarning:streamlit.runtime.caching.cache_data_api,ignore::UserWarning:streamlit.runtime.scriptrunner_utils.script_run_context
...
validate-mobile: check-venv
	...
	@$(PY) -c "import logging, warnings; logging.basicConfig(level=logging.ERROR); logging.getLogger().setLevel(logging.ERROR); [logging.getLogger(name).setLevel(logging.ERROR) for name in ('streamlit', 'streamlit.runtime', 'streamlit.runtime.caching.cache_data_api', 'streamlit.runtime.scriptrunner_utils.script_run_context')]; warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.caching.cache_data_api'); warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.scriptrunner_utils.script_run_context'); import mobile_spa_app; print('[DONE] Mobile SPA imports successful')" || { \
		echo "$(RED)[TODO] Mobile SPA validation failed$(NC)"; \
		echo "$(CYAN)[TIP] Check mobile component dependencies$(NC)"; \
		exit 1; \
	}
	@$(PY) -c "import logging, warnings; logging.basicConfig(level=logging.ERROR); logging.getLogger().setLevel(logging.ERROR); [logging.getLogger(name).setLevel(logging.ERROR) for name in ('streamlit', 'streamlit.runtime', 'streamlit.runtime.caching.cache_data_api', 'streamlit.runtime.scriptrunner_utils.script_run_context')]; warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.caching.cache_data_api'); warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.scriptrunner_utils.script_run_context'); import streamlit; print('[DONE] Streamlit available')" || { echo "$(RED)[TODO] Streamlit not found$(NC)"; exit 1; }
	@$(PY) -c "import logging, warnings; logging.basicConfig(level=logging.ERROR); logging.getLogger().setLevel(logging.ERROR); [logging.getLogger(name).setLevel(logging.ERROR) for name in ('streamlit', 'streamlit.runtime', 'streamlit.runtime.caching.cache_data_api', 'streamlit.runtime.scriptrunner_utils.script_run_context')]; warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.caching.cache_data_api'); warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.scriptrunner_utils.script_run_context'); import torch; print(f'[DONE] PyTorch available: {torch.__version__}')" || { echo "$(RED)[TODO] PyTorch not found$(NC)"; exit 1; }
	@$(PY) -c "import logging, warnings; logging.basicConfig(level=logging.ERROR); logging.getLogger().setLevel(logging.ERROR); [logging.getLogger(name).setLevel(logging.ERROR) for name in ('streamlit', 'streamlit.runtime', 'streamlit.runtime.caching.cache_data_api', 'streamlit.runtime.scriptrunner_utils.script_run_context')]; warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.caching.cache_data_api'); warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.scriptrunner_utils.script_run_context'); import PIL; print('[DONE] PIL available')" || { echo "$(RED)[TODO] PIL not found$(NC)"; exit 1; }
	@$(PY) -c "import logging, warnings; logging.basicConfig(level=logging.ERROR); logging.getLogger().setLevel(logging.ERROR); [logging.getLogger(name).setLevel(logging.ERROR) for name in ('streamlit', 'streamlit.runtime', 'streamlit.runtime.caching.cache_data_api', 'streamlit.runtime.scriptrunner_utils.script_run_context')]; warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.caching.cache_data_api'); warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.scriptrunner_utils.script_run_context'); from plantguard.ui.components.mobile_component_registry import mobile_component_registry; print('[DONE] Mobile components available')" || { \
		echo "$(YELLOW)[WARNING]  Mobile components not fully available$(NC)"; \
	}
	@$(PY) -c "import logging, warnings; logging.basicConfig(level=logging.ERROR); logging.getLogger().setLevel(logging.ERROR); [logging.getLogger(name).setLevel(logging.ERROR) for name in ('streamlit', 'streamlit.runtime', 'streamlit.runtime.caching.cache_data_api', 'streamlit.runtime.scriptrunner_utils.script_run_context')]; warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.caching.cache_data_api'); warnings.filterwarnings('ignore', category=UserWarning, module='streamlit.runtime.scriptrunner_utils.script_run_context'); from plantguard.ui.components.mobile_testing_framework import MobileTestingFramework; print('[DONE] AI testing framework available')" || { \
