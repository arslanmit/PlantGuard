...
def _load_checkpoint_for_validation(checkpoint_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.debug("Safe validation retry failed for %s", checkpoint_path, exc_info=True)
            raise CheckpointIntegrityError("Checkpoint could not be safely loaded for validation") from retry_error
