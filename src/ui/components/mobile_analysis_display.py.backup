...
class MobileAnalysisDisplay(MobileBaseComponent):
        ...
        for i, result in enumerate(results[: self.display_config["max_results_shown"]]):
            ...
            with st.expander(f"🌿 {disease_name} ({confidence:.1%}) - {self._format_timestamp(timestamp)}", expanded=(i == 0)):
                ...
                with col2:
                    ...
                    if st.button("🔄 Re-analyze", key=f"{self.component_id}_reanalyze_{i}", use_container_width=True):
    ...
    def _render_recommendations(self, disease_name: str, confidence: float) -> None:
        ...
        if disease_info and "treatment" in disease_info:
            ...
            if confidence_level == "high":
                st.success("✅ High confidence diagnosis - Follow specific treatment guidelines")
            elif confidence_level == "medium":
                st.warning("⚠️ Medium confidence - Consider consulting a plant pathologist")
            else:
                st.error("❌ Low confidence - Expert consultation strongly recommended")
    ...
    def _render_result_actions(self, result: dict[str, Any]) -> None:
        ...
        with col3:
            if st.button("🔄 Re-analyze", key=f"{self.component_id}_reanalyze"):
                self._reanalyze_result(result)

        with col4:
            if st.button("❌ Clear", key=f"{self.component_id}_clear"):
                self._clear_results()
...
Generated by PlantGuard AI Plant Disease Detection
        ...
        st.success("✅ Result ready to share! Copy the text above.")
        ...
        if "image" in result:
            try:
                ...
                with st.spinner("🔄 Re-analyzing image..."):
                    ...
                    st.success(f"🔄 Re-analysis complete: {disease_name} ({confidence:.1%})")

            except Exception as e:
                logger.error("Re-analysis failed: %s", e)
                st.error("❌ Re-analysis failed. Please try again.")
        else:
            st.warning("⚠️ No image available for re-analysis.")
