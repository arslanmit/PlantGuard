...
class MobileVoiceInterface(MobileComponent):
    ...
    def _get_component_metadata(self) -> ComponentMetadata:
        ...
        return ComponentMetadata(
            ...
            test_scenarios=[
                ...
                {
                    'name': 'audio_upload',
                    'description': 'Test audio file upload',
                    'expected_outcome': 'Audio files upload and process correctly'
                },
                {
                    'name': 'transcription',
                    'description': 'Test voice transcription',
                    'expected_outcome': 'Audio transcribes to text accurately'
                },
                {
    ...
    def render_voice_recording_section(self) -> Optional[bytes]:
        ...
        with record_tab:
            ...
            audio_data = st.audio_input(
                ...
                key=f"{self.component_id}_recorder",
                help=f"Record up to {self.max_recording_duration} seconds"
            )
...
For specific advice about your plant's symptoms or care needs, consider consulting with a local plant expert or uploading a photo for visual analysis."""
    ...
    def render(self, **kwargs) -> Dict[str, Any]:
        ...
        # Main container
        st.markdown('<div class="mobile-voice-interface" data-component="mobile-voice-interface" data-testable="true">', 
...
# Utility functions
def create_mobile_voice_interface(max_recording_duration: int = 60, 
                                 auto_transcribe: bool = True) -> MobileVoiceInterface:
    """Create and return a MobileVoiceInterface instance."""
    return MobileVoiceInterface(
        component_id="mobile_voice_interface",
