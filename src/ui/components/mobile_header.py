...
class MobileHeader(MobileComponent):
        ...
        return ComponentMetadata(
            ...
            test_scenarios=[
                ...
                },
                {
                    ...
                    'description': 'Test model selection works',
                    'expected_outcome': 'Model changes when selected'
                },
                {
                    'name': 'status_display',
                    'description': 'Test status indicator updates',
                    'expected_outcome': 'Status reflects system state'
                }
            ],
    ...
    def render_model_switcher(self) -> None:
        ...
        for i, model_key in enumerate(model_options):
            with cols[i]:
                ...
                short_names = {
                    'vit_best': 'ViT',
                    'resnet50_plantvillage_v1': 'ResNet', 
                    'mobilenet_fast': 'Mobile'
                }
        ...
        with col2:
            if self.show_model_switcher:
                ...
                if st.button(f"🤖 {model_name}", 
                           key=f"{self.component_id}_compact_model",
                           use_container_width=True):
        ...
        with st.expander("🤖 Select AI Model", expanded=True):
            ...
            for model_key in model_options:
                ...
                if st.button(button_text, 
                           key=f"{self.component_id}_modal_{model_key}",
                           use_container_width=True,
                           disabled=is_current):
                    if not is_current:
                        st.session_state.current_vision_model = model_key
    ...
    def _get_model_info(self, model_key: str) -> Dict[str, str]:
        ...
        return model_details.get(model_key, {
            'name': 'Unknown Model',
            'accuracy': 'N/A',
            'speed': 'N/A',
...
# Utility functions
def create_mobile_header(title: str = "PlantGuard AI", 
                        subtitle: str = "Mobile Plant Disease Detection",
                        show_model_switcher: bool = True,
                        show_status: bool = True) -> MobileHeader:
    """Create and return a MobileHeader instance."""
    return MobileHeader(
        component_id="mobile_header",
        title=title,
        subtitle=subtitle,
        show_model_switcher=show_model_switcher,
