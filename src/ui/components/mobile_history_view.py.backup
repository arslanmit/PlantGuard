...
class MobileHistoryView:
    ...
    def render_history_card(self, item: dict[str, Any], index: int) -> None:
        ...
        with col1:
            if st.button("👁️ View", key=f"{self.component_id}_view_{index}", use_container_width=True):
                self._handle_view_item(item, index)

        with col2:
            if st.button("📤 Share", key=f"{self.component_id}_share_{index}", use_container_width=True):
                self._handle_share_item(item, index)

        with col3:
            if st.button("🗑️ Delete", key=f"{self.component_id}_delete_{index}", use_container_width=True):
    ...
    def render_empty_state(self) -> None:
        ...
        <div class="mobile-history-empty">
            <div class="mobile-history-empty-icon">📊</div>
    ...
    def _handle_view_item(self, item: dict[str, Any], index: int) -> None:
        ...
        with st.expander(f"📊 Analysis Details - {item.get('prediction', 'Unknown')}", expanded=True):
        ...
        st.toast(f"Viewing analysis: {item.get('prediction', 'Unknown')}", icon="👁️")
    ...
    def _handle_share_item(self, item: dict[str, Any], index: int) -> None:
        ...
        share_text = f"""🌿 PlantGuard Analysis Result
...
Generated by PlantGuard Mobile App"""
        ...
        # Show share options
        with st.expander(f"📤 Share Analysis - {prediction}", expanded=True):
            st.text_area("Share this analysis:", value=share_text, height=150, key=f"{self.component_id}_share_text_{index}")

            col1, col2 = st.columns(2)
            with col1:
                if st.button("📋 Copy to Clipboard", key=f"{self.component_id}_copy_{index}", use_container_width=True):
        ...
        st.toast(f"Sharing options for: {prediction}", icon="📤")
    ...
    def _handle_delete_item(self, item: dict[str, Any], index: int) -> None:
        ...
        if not st.session_state[confirm_key]:
            ...
            col1, col2 = st.columns(2)
            with col1:
                if st.button("✅ Yes, Delete", key=f"{self.component_id}_confirm_yes_{index}", use_container_width=True):
                    # Remove item from history
                    history = st.session_state.get("analysis_history", [])
                    if index < len(history):
                        removed_item = history.pop(index)
                        st.session_state.analysis_history = history
                        st.toast(f"Deleted analysis: {removed_item.get('prediction', 'Unknown')}", icon="🗑️")
                        st.session_state[confirm_key] = False
                        st.rerun()

            with col2:
                if st.button("❌ Cancel", key=f"{self.component_id}_confirm_no_{index}", use_container_width=True):
                    st.session_state[confirm_key] = False
                    st.rerun()
