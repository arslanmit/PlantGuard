...
def predict(model, input_batch, class_names):
    ...
    return [
        {'class': class_names[i], 'probability': prob.item()}
        for i, prob in zip(top5_catid, top5_prob)
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='data/models/converted_model_fixed.pt',
    ...
    output_file = f"runs/inference/{os.path.basename(args.image).split('.')[0]}_results.json"
