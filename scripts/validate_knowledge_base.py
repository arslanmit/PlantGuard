...
def validate_knowledge_base():
    ...
    for class_name, disease_info in diseases.items():
        ...
        if missing_fields:
            print(f"❌ {class_name} missing fields: {missing_fields}")
        else:
            valid_count += 1

    print(f"✅ Validated {valid_count}/{len(diseases)} disease entries")
    print(f"✅ Knowledge base covers all {len(expected_classes)} PlantVillage classes")
    ...
    for class_name, disease_info in diseases.items():
        ...
        if not isinstance(treatment, dict):
            print(f"❌ {class_name}: treatment should be a dictionary")
            treatment_issues += 1
            continue
        ...
        for key in required_treatment_keys:
            if key not in treatment:
                print(f"❌ {class_name}: missing treatment.{key}")
                treatment_issues += 1

    if treatment_issues == 0:
        print("✅ All treatment structures are valid")
    else:
        print(f"❌ Found {treatment_issues} treatment structure issues")
