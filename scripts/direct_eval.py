...
def main() -> None:
    ...
    # Choose dataset (processed preferred, fallback to legacy)
    for candidate in ["data/processed/plantvillage", "data/PlantVillage"]:
        cand_path = Path(candidate)
        if cand_path.joinpath("val").exists() or cand_path.joinpath("train").exists():
            data_dir = str(cand_path)
            break
    else:
