...
def init_session_state():
    ...
    if "current_models" not in st.session_state:
        st.session_state.current_models = {"vision": "vit_best", "audio": "whisper_tiny_local", "text": "distilbert_plant_qa_v1"}
...
class PlantGuardSPA:
    ...
    def __init__(self):
        ...
        self.models = {
            "vision": {
                "vit_best": {"name": "Vision Transformer (Best)", "accuracy": "100%", "speed": "Medium", "description": "Highest accuracy model"},
                "resnet50_plantvillage_v1": {"name": "ResNet50", "accuracy": "95%", "speed": "Fast", "description": "Balanced performance"},
                "mobilenet_fast": {"name": "MobileNet", "accuracy": "90%", "speed": "Very Fast", "description": "Lightweight for mobile"},
            },
            "audio": {"whisper_tiny_local": {"name": "Whisper Tiny", "accuracy": "85%", "speed": "Fast"}},
            "text": {"distilbert_plant_qa_v1": {"name": "DistilBERT", "accuracy": "92%", "speed": "Fast"}},
        }
    ...
    def _setup_performance_monitoring(self):
        ...
        self.performance_metrics = {"start_time": time.time(), "analysis_times": [], "memory_usage": [], "model_load_times": {}}
    ...
    def clear_caches(self):
        ...
        if "preprocessed_images" in st.session_state:
    ...
    def handle_error(self, error: Exception, context: str, fallback_action: str = None) -> bool:
        ...
        self.logger.error(f"Error {error_id} in {context}: {error!s}", exc_info=True)

        # Update session state
        self.update_processing_state("error", error_msg=f"{context}: {error!s}")
...
Session ID: {id(st.session_state)}
    ...
    def setup_error_monitoring(self):
        ...
        if "error_stats" not in st.session_state:
            st.session_state.error_stats = {"total_errors": 0, "error_types": {}, "recovery_success_rate": 0, "last_error_time": None}
    ...
    def render_mobile_optimized_input(self):
        ...
        with tab1:
            ...
            uploaded_file = st.file_uploader("Upload plant image", type=["jpg", "jpeg", "png"], accept_multiple_files=True, key="mobile_image_upload")
        ...
        with tab2:
            ...
            if st.session_state.get("show_audio_upload", False):
                audio_file = st.file_uploader("Audio file", type=["wav", "mp3", "m4a", "ogg"], key="mobile_audio_upload")
                if audio_file:
        ...
        with tab3:
            ...
            text_query = st.text_area("Ask about plant care", placeholder="What's wrong with my plant?", height=100, key="mobile_text_input")
    ...
    def render_adaptive_layout(self):
        ...
        # self.render_accessibility_features()  # Removed accessibility options
    ...
    def analyze_image_programmatic(self, image_path: str, model: str = None) -> dict[str, Any]:
        ...
        try:
            ...
            if result:
                ...
                return {"status": "error", "error": "Analysis failed", "timestamp": datetime.now().isoformat()}
    ...
    def process_voice_programmatic(self, audio_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not adapter:
                return {"status": "error", "error": "Audio adapter not available", "timestamp": datetime.now().isoformat()}
            ...
            if result:
                ...
                return {"status": "error", "error": "Audio processing failed", "timestamp": datetime.now().isoformat()}
    ...
    def query_programmatic(self, query: str, context: dict = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not adapter:
                return {"status": "error", "error": "Text adapter not available", "timestamp": datetime.now().isoformat()}

            # Generate response
            response = adapter.generate_response(disease_class="general", user_query=query)
    ...
    def get_system_status_programmatic(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "statistics": {"total_analyses": total_analyses, "session_active": True},
            }
    ...
    def render_primary_input_zone(self):
        ...
        with input_col2:
            ...
            if st.session_state.get("show_audio_upload", False):
                audio_file = st.file_uploader(
                    "Upload audio file", type=["wav", "mp3", "m4a", "ogg"], key="audio_file_upload", help="Supported: WAV, MP3, M4A, OGG"
                )
                if audio_file:
                    self.process_audio_file(audio_file)
                    st.session_state.show_audio_upload = False
        ...
        suggestions = ["What disease is this?", "How to treat this?", "Watering schedule?", "Prevention tips?"]
    ...
    def render_model_selector(self):
        ...
        selected_vision = st.selectbox("Vision", options=vision_names, index=current_idx, key="vision_model_select")
        ...
        if st.button("🔄 Analyze All Images", type="primary", use_container_width=True):
            ...
            for i, file in enumerate(uploaded_files):
                ...
                with st.spinner(f"Analyzing {file.name}..."):
                    ...
                    if result:
                        # Add to history
                        result.update({"timestamp": datetime.now().isoformat(), "filename": file.name, "type": "batch_image"})
    ...
    def process_text_query(self, query: str):
        ...
        # Add to chat history
        st.session_state.chat_messages.append({"role": "user", "content": query, "timestamp": datetime.now().isoformat()})
        ...
        if adapter:
            with st.spinner("🤔 Thinking..."):
                ...
                st.session_state.chat_messages.append({"role": "assistant", "content": response, "timestamp": datetime.now().isoformat()})

        # Clear input and rerun to show new messages
        # Note: Cannot modify st.session_state.main_text_input after widget is instantiated
        # The input will be cleared automatically after processing
        st.rerun()
    ...
    def analyze_image(self, image: Image.Image, filename: str):
        ...
        try:
            ...
            if result:
                # Add to history
                result.update({"timestamp": datetime.now().isoformat(), "filename": filename, "type": "image"})
                st.session_state.analysis_history.append(result)
    ...
    def perform_image_analysis(self, image: Image.Image) -> dict[str, Any] | None:
        ...
        if st.session_state.analysis_history:
            ...
            for i, analysis in enumerate(reversed(st.session_state.analysis_history[-5:])):
                with st.expander(f"{analysis.get('disease', 'Unknown')} - {analysis.get('confidence', 0):.1%}", expanded=False):
                    ...
                    if st.button("🔄 View Details", key=f"history_detail_{i}"):
    ...
    def get_treatment_recommendations(self, disease: str) -> list[str]:
        ...
        recommendations = {
            ...
            "Bacterial Spot": ["Remove infected plant parts", "Apply copper spray", "Space plants for airflow", "Water at soil level"],
            "Leaf Mold": ["Increase ventilation", "Reduce humidity", "Apply fungicide if severe", "Remove affected leaves"],
        }

        return recommendations.get(
            disease,
            ["Consult local agricultural extension", "Isolate affected plants", "Monitor plant health closely", "Consider professional diagnosis"],
        )
    ...
    def render_comparison_overlay(self):
        ...
        # Use unique timestamp-based keys to prevent duplicates on re-renders
        import time

        key_suffix = str(int(time.time() * 1000) % 10000)  # Last 4 digits of millisecond timestamp

        comp_col1, comp_col2, comp_col3 = st.columns([1, 1, 1])

        with comp_col1:
            st.markdown("**📷 Image 1:**")
            comp_file1 = st.file_uploader("First image", type=["jpg", "jpeg", "png"], key=f"comp1_overlay_{key_suffix}")

        with comp_col2:
            st.markdown("**📷 Image 2:**")
            comp_file2 = st.file_uploader("Second image", type=["jpg", "jpeg", "png"], key=f"comp2_overlay_{key_suffix}")

        with comp_col3:
            st.markdown("**⚙️ Options:**")
            if st.button("🔄 Compare", disabled=not (comp_file1 and comp_file2), key=f"perform_comparison_{key_suffix}"):
                self.perform_comparison(comp_file1, comp_file2)

            if st.button("❌ Exit Comparison", key=f"exit_comparison_{key_suffix}"):
                st.session_state.comparison_mode = False
                st.rerun()
    ...
    def render_performance_metrics(self):
        ...
        try:
            ...
            # Memory usage
            memory = psutil.virtual_memory()
            st.metric("💾 Memory", f"{memory.percent:.1f}%", delta=f"{memory.available / (1024**3):.1f}GB free")
                    ...
                    if all(times):
                        try:
                            ...
                            if len(parsed_times) >= 2:
                                time_diffs = [(parsed_times[i] - parsed_times[i - 1]).total_seconds() for i in range(1, len(parsed_times))]
    ...
    def render_api_status(self):
        ...
        adapters_status = {"Vision": self.vision_adapter is not None, "Audio": self.audio_adapter is not None, "Text": self.text_adapter is not None}
        ...
        total_api_calls = len(st.session_state.get("analysis_history", [])) + len(st.session_state.get("chat_messages", []))
    ...
    def update_processing_state(self, state: str, processing_type: str = None, batch_info: dict = None, error_msg: str = None):
        ...
        # Track performance
        performance_entry = {"timestamp": datetime.now().isoformat(), "state": state, "type": processing_type, "batch_info": batch_info}
    ...
    def render_comparison_mode(self):
        ...
        with col_comp1:
            st.markdown("**📷 Image 1:**")
            comp_file1 = st.file_uploader("First image", type=["jpg", "jpeg", "png"], key="comp1_mode")

        with col_comp2:
            st.markdown("**📷 Image 2:**")
            comp_file2 = st.file_uploader("Second image", type=["jpg", "jpeg", "png"], key="comp2_mode")
